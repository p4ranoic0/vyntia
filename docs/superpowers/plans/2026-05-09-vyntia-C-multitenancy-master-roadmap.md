...
| **C.4** | `vyntia/C4-auth-tenant-aware` | Backend (Auth) | Tenant-aware login (membership lookup via RLS). Activation endpoint (`POST /auth/activate/`). Workspace listing (`GET app.vyntia.pe/api/v1/workspaces/`) and exchange flow. JWT claims include `tenant_id`, `tenant_slug`, `membership_role`. | `2026-05-09-vyntia-C4-auth-tenant-aware.md` ✅ merged 2026-05-09 |
