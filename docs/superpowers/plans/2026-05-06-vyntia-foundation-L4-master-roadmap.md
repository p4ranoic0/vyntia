...
| **L4.10** | onboarding feature completion | `vyntia/L4.10-feature-onboarding` | Moved 3 files (OnboardingAdminPage + OnboardingPage + onboardingService) → existing `features/onboarding/`. Extended pages/index.ts + services/index.ts barrels (preserved OnboardingEmployeePage + onboardingUploadService entries; added onboardingDataService too for completeness). 4 import sites redirected. Sibling sweep clean. 3 atomic commits. Pytest 161/8/3 + vitest 7 + build clean (✓ 8.16s) + tsc 1 + lint 634 baselines preservados. Rename detection 99-100%. | ~8 | Bajo | L4.1 | ✅ merged `4eb162a9` |
| **L4.11** | layout + final cleanup | `vyntia/L4.11-layout-cleanup` | Moved 10 files (7 layout components + NotificationsBell → `shared/layout/`, useMenu → `shared/hooks/`, menuService → `shared/api/`). Deleted 11 dirs: 7 Spanish placeholder feature dirs (admin/areas/contratos/legajo/security/usuarios/vacaciones — features/empleados/ already removed in L4.5) + 4 empty structural dirs (components/layout, components/notifications, components/, services/). ~16 import sites updated across 16 files. CLAUDE.md updated with new path references. Sibling sweep clean. 3 atomic commits. Pytest 161/8/3 + vitest 7 + build clean (✓ 9.11s) + tsc 1 (BlankEnum) + lint 634 baselines preservados. **L4 COMPLETE — all 11 sub-PRs merged.** | ~15 | Medio | L4.2-L4.10 | ✅ merged `c88a86bd` |
