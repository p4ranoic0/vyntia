...
| **L4.6** | contracts feature migration | `vyntia/L4.6-feature-contracts` | Move `pages/contratos/ContratosPage.tsx` → `features/contracts/pages/`. Move `services/contractsService.ts` → `features/contracts/services/`. Update routes for /contratos/. Delete `features/contratos/` placeholder. ~15-20 import updates | ~8 | Bajo | L4.1 | ⏳ NEXT |
