...
| **L4.3** | identity feature migration | `vyntia/L4.3-feature-identity` | Move `pages/users/*` → `features/identity/pages/`. Move `pages/security/*` → `features/identity/pages/`. Move `components/users/modals/*` → `features/identity/components/modals/`. Move `services/{usersService,securityService}.ts` → `features/identity/services/`. Update App.tsx routes for /usuarios/, /seguridad/. ~50-70 import updates | ~18 | Medio | L4.1 | ⏳ NEXT |
