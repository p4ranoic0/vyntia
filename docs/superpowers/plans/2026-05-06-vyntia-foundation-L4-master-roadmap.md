...
|---|---|---|---|---|---|---|:---:|
| **L4.1** | shared/ infrastructure consolidation | `vyntia/L4.1-shared-infra` | Move `lib/api.ts`+`lib/queryClient.ts`+`lib/errorUtils.ts`+`services/normalizers/` → `shared/api/`. Move `components/ui/` (shadcn 30+ files) → `shared/ui/`. Move `components/brand/` → `shared/ui/brand/`. Move `components/common/` → `shared/components/`. Move `hooks/{useDebounce,use-toast,useAnimations,useScrolled}` → `shared/hooks/`. Move `utils/` → `shared/utils/`. Move `lib/utils.ts`+`lib/design-tokens.ts` → `shared/utils/`. Update tsconfig path aliases. ~150-200 import updates across consumers (replace_all per file) | ~30 | Medio | L3.11 ✅ | ⏳ NEXT |
