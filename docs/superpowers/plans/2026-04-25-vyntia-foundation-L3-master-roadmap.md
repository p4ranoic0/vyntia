...
| L3.10.4 | Frontend rename | `vyntia/L3.10.4-frontend-rename` | 14 services TS (`contratosService`→`contractsService`, etc.) + tipos TS (`empleado_id: number` → `id: string` UUID, `fecha_creacion → created_at`) + URL paths nuevos `/api/v1/employees/`, `/api/v1/contracts/`, `/api/v1/contracts/{id}/amendments/` (separar `/adendas/` que está en `/api/v1/rrhh/adendas/` ahora). Coordinar con L4 frontend reorg | ~15 | Bajo-medio | L3.10.3 | ⏳ NEXT |
