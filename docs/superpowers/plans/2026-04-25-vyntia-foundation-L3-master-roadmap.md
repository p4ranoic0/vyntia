...
| L3.10.4 | Frontend rename | (split en 2 sub-PRs: 4a + 4b) | URL refactor backend + frontend rename. Dividido para reducir scope por PR | — | — | L3.10.3 | ⏳ EN PROGRESO |
...
| L3.10.4b | Frontend rename | `vyntia/L3.10.4b-frontend-rename` | 14 services TS (`contratosService`→`contractsService`, etc.) + tipos TS (`empleado_id: number` → `id: string` UUID, `fecha_creacion → created_at`) + URL constants ahora apuntan a `/api/v1/employees/`, `/api/v1/contracts/`, etc. ~726 audit/PK refs + 154 URL legacy paths + 179 state field refs + 50 component refs | ~25 | Medio | L3.10.4a | ⏳ NEXT |
