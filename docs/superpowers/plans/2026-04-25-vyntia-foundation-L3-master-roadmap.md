...
| L3.10.2 | Field names ES→EN | `vyntia/L3.10.2-rename-fields` | Renombrar fields (`nombres`→`first_name`, `apellido_paterno`→`last_name`, `fecha_nacimiento`→`birth_date`, `creado_en`→`created_at`, `estado`→`status`, `activo`→`is_active`, `creado_por`→`created_by`, etc. per spec § 3.6) preservando `db_column` español. `RenameField` por field. PK rename `empleado_id`/`contrato_id`→`id` UUID también aquí | ~25 | Medio-alto | L3.10.1 | ⏳ NEXT |
| L3.10.3 | Model splits | `vyntia/L3.10.3-splits` | `Contract`→`Contract`+`ContractAmendment` (partition by tipo); `VacationRequest`→`VacationRequest`+`VacationBalance`. Data migrations vía `RunPython` | ~15 | **Crítico** (data migration) | L3.10.2 | ⏳ |
| L3.10.4 | Frontend rename | `vyntia/L3.10.4-frontend-rename` | 14 services TS (`contratosService`→`contractsService`, etc.) + tipos TS + URL paths nuevos `/api/v1/employees/`. Coordinar con L4 frontend reorg | ~15 | Bajo-medio | L3.10.3 | ⏳ |
