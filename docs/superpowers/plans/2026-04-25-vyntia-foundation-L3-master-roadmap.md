...
| L3.10.3 | Model splits | `vyntia/L3.10.3-splits` | `Contract`→`Contract`+`ContractAmendment` (partition by tipo); `VacationRequest`→`VacationRequest`+`VacationBalance`. Data migrations vía `RunPython` | ~15 | **Crítico** (data migration) | L3.10.2 | ⏳ NEXT |
