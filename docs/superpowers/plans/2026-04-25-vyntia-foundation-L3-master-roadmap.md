...
| L3.10 | Spanish → English rename | `vyntia/L3.10-rename-en` | Aplicar tabla del spec § 3.6: `nombres`→`first_name`, `Empleado`→`Employee`, etc. Migration `RenameModel` + `AlterField` masiva. **Frontend services + tipos TS también**. Incluye splits diferidos: `ContratosAdendas → Contract + ContractAmendment`, `SolicitudVacaciones → VacationRequest + VacationBalance` | ~30+ | **Crítico** (toca todo) | L3.1–L3.9 | ⏳ NEXT |
