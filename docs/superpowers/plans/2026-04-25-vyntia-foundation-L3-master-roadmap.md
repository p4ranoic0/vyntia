...
| L3.10.4d | Frontend state fields rename | `vyntia/L3.10.4d-frontend-state-fields` | TS interfaces con backend platform state: `estado: string`→`status: string`, `activo: boolean`→`is_active: boolean`. SOLO donde corresponde a backend platform state. Preservar domain Spanish (`estado_empleado`, `estado_civil`, `vigencia_estado_seguro`, `estado_documento`, `estado_familiar`, `estado_ubicacion`, `estado_datos`, `estado_solicitud`) y local UI state / labels / design tokens (`useLoading.estado`, `LegajoPage.estado` label key, `design-tokens.activo`). Requiere lectura per-archivo, no replace_all blindo | ~20 | Medio | L3.10.4c | ⏳ NEXT |
| L3.10.4e | Frontend PK type change | `vyntia/L3.10.4e-frontend-pk-uuid` | TS interfaces: `<entity>_id: number` → `id: string` UUID. ~57 declarations + ~252 usage refs. Highest semantic risk: function signatures, comparisons, React keys, URL building, `parseInt`/`Number` calls deben removerse. Genera nuevas conversiones donde algún componente local hardcodee assumption de `number` | ~25 | Alto | L3.10.4d | ⏳ |
| L3.10.4f | Frontend file renames | `vyntia/L3.10.4f-frontend-file-renames` | `git mv contratosService.ts contractsService.ts`, `areasService.ts → departmentsService.ts`, `empresaService.ts → companyService.ts`, `vacacionesService.ts → timeOffService.ts`, `remuneracionesService.ts → payrollService.ts`, `plantillasService.ts → templatesService.ts`, `legajoService.ts` (preservado — domain term), `services/normalizers/rrhhNormalizers.ts → identityNormalizers.ts` (o split por dominio). 82 imports en consumers actualizados. `generated/api/services/` regenerado del schema actualizado | ~12 | Bajo | L3.10.4e | ⏳ |
