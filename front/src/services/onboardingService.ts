...
export const onboardingService = {
  ...
  async validar(id: number, accion: 'aprobar' | 'rechazar', observaciones?: string): Promise<OnboardingStatus> {
    const response = await apiClient.post(`/api/v1/rrhh/onboarding/${id}/validar/`, {
      accion,
  ...
  async actualizarEstado(id: number): Promise<OnboardingStatus> {
    const response = await apiClient.post(`/api/v1/rrhh/onboarding/${id}/actualizar-estado/`, {})
    return response.data?.data || response.data
  },
}
