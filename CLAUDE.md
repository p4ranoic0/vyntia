...
D:\VYNTIA\
├── apps/
│   ├── api/              # Django 4.2 + DRF backend (was D:\INTRANET\back\)
│   │   ├── manage.py
│   │   ├── config/       # settings module — rename to vyntia/ in L1
...
│   └── web/              # React 18 + TS + Vite frontend (was D:\INTRANET\front\)
...
Active settings via `DJANGO_SETTINGS_MODULE=config.settings.development`. Tests use `config.settings.testing`.
**L1 plan:** rename `config/` → `vyntia/`, settings module → `vyntia.settings.*`.
...
PostgreSQL `bd_rrhh_intranet` on localhost:5432 (legacy). Override via env: `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `DB_HOST`, `DB_PORT`.
...
- L1 ⏳ Rebrand superficial (BD `bd_vyntia`, `config/`→`vyntia/`, design tokens VYNTIA, Inter font)
